create table assignment (id integer not null auto_increment, deltaOverhead integer, backlog_id integer, user_id integer, primary key (id)) ENGINE=InnoDB;
create table backlog (backlogtype varchar(31) not null, id integer not null auto_increment, name varchar(255), description text, startDate datetime, endDate datetime, defaultOverhead integer, rank integer not null default 0 not null, projectType_id integer, assignee_id integer, product_id integer, owner_id integer, history_fk integer not null, project_id integer, primary key (id), unique (history_fk)) ENGINE=InnoDB;
create table backlogitem (id integer not null auto_increment, effortLeft integer, originalEstimate integer, name varchar(255), priority integer, state integer, description text, backlog_id integer not null, iterationGoal_id integer, assignee_id integer, primary key (id)) ENGINE=InnoDB;
...
create table task (id integer not null auto_increment, name varchar(255), priority integer, state integer, rank integer, description text, backlogItem_id integer not null, creator_id integer, primary key (id)) ENGINE=InnoDB;
