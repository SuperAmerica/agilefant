...
create table Backlog (backlogtype varchar(31) not null, id integer not null auto_increment, name varchar(255), description text, startDate datetime, endDate datetime, defaultOverhead integer, rank integer not null default 0 not null, product_id integer, history_fk integer, projectType_id integer, project_id integer, assignee_id integer, owner_id integer, primary key (id)) ENGINE=InnoDB;
create table BacklogItem (id integer not null auto_increment, effortLeft integer, originalEstimate integer, name varchar(255), priority integer, state integer, description text, backlog_id integer not null, assignee_id integer, iterationGoal_id integer, primary key (id)) ENGINE=InnoDB;
