 ...
 */
public class TaskEventDAOHibernate extends GenericDAOHibernate<TaskEvent>
		implements TaskEventDAO {
	...
	public AFTime getBLIOriginalEstimate(BacklogItem backlogItem, Date date) {
		...
		String query = "select b.placeHolder.created " + "from BacklogItem b "
		...
		/*
		 * If bli is created bafore given date use the closest effort estimate
		 * event to the given date that is created before given date
		 */
		if (creationDate.before(date)) {
			...
			query3 = "from EstimateHistoryEvent e, BacklogItem b "
					...
					+ "b = :backlogItem and " + "e.created <= :date and "
					+ "e.newEstimate != null";
			...
			resultList = (List<AFTime>) ht.findByNamedParam(query, queryParams,
					queryValues);
		}
		/*
		 * If no result is found or creation date is after the given date use
		 * the first event in this iteration.
		 */
		if (resultList == null || resultList.isEmpty()
				|| resultList.get(0) == null || !creationDate.before(date)) {
			queryParams = new String[] { "backlogItem" };
			queryValues = new Object[] { backlogItem };
			query3 = "from EstimateHistoryEvent e, BacklogItem b "
					+ "where e.task = b.placeHolder "
					+ "and b = :backlogItem and " + "e.newEstimate != null";
	...
	public AFTime getTaskSumOrigEst(BacklogItem backlogItem, Date date) {
		...
		query = "from Task t " + "where t.backlogItem = :backlogItem"
				+ placeholderQuery;

		realTasks = (List<Task>) ht.findByNamedParam(query, queryParams,
				queryValues);
		...
		for (Task task : realTasks) {
			if (task.getCreated().before(date)) {
				...
				query3 = "from EstimateHistoryEvent e "
						...
						+ "e.created <= :date and " + "e.newEstimate != null";

				query2 = "select max(created) " + query3 + "";

				query = "select f.newEstimate from EstimateHistoryEvent f "
						+ "where f.created = (" + query2 + ") and "
						+ "f.task = :task " + "order by f.newEstimate desc";
				resultList = (List<AFTime>) ht.findByNamedParam(query,
						queryParams, queryValues);
			} else {
				queryParams = new String[] { "task" };
				queryValues = new Object[] { task };
				query3 = "EstimateHistoryEvent e "
						+ "where e.task = :task and " + "e.newEstimate != null";
