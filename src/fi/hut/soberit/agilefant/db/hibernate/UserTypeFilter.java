 ...
 * modifying data incoming/outgoing from/to the database on the fly. It's invisible 
 * for the entity bean user.
 * <p>
 * Technically, this is a UserType, which delegates all UserType functionality to 
 * some other UserType, but modifying the data passing trough. Subclasses
 * may define the filtering functionality in methods filterDown and filterUp. Chaining
 * the filters is possible.
 * <p>
...
public abstract class UserTypeFilter implements UserType, ParameterizedType {
    ...
    /**
     * Subclass implemented method, with modifies the data 
     * coming "up" in the UserType hierarchy, ie. from the database.
     * 
     * @param ob value to modify
    ...
    /**
     * Subclass implemented method, with modifies the data 
     * going "down" in the UserType hierarchy, ie. to the database.
     * 
     * @param ob value to modify
     * @return modified value
     */
    ...
    // "UserType" methods, which are implemented by delegating everything to subUserType.
    // All data passed down to subUserType is filtered with FilterDown. All data from 
    // the subUserType is filtered with filterUp.
    /////////////
    
    public int[] sqlTypes() { return subUserType.sqlTypes(); }    
    public Class returnedClass() { return subUserType.returnedClass(); }
    public boolean isMutable() { return subUserType.isMutable(); }
