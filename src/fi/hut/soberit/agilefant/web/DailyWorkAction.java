	...
	private void generateIterations() {
		...
		Iterator<Iteration> iIt = ongoingIterations.iterator();
		 while (iIt.hasNext()) {
			 ...
			 Collection<BacklogItem> blItems = currentIteration.getSortedBacklogItems();
			 ...
			 while (bliIt.hasNext()) {
				 ...
				 if(currentBli.getAssignee() != null &&
						 ...
						 && currentBli.getPlaceHolder().getStatus()
						 != fi.hut.soberit.agilefant.model.TaskStatus.DONE) {
					 unfinishedBlis.add(currentBli);
				 }
			 }

			 if(!unfinishedBlis.isEmpty()) {
				 BacklogValueInjector.injectMetrics(currentIteration, currentIteration.getStartDate(), 
