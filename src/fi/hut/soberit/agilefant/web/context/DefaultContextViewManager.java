...
public class DefaultContextViewManager implements ContextViewManager {
	...
	private static final Logger log = Logger.getLogger(DefaultContextViewManager.class);
	...
	public void setParentContext(ContextView context) {
		if (context != null){
			...
			log.debug("Parent context set to: name=" + context.getContextName() + ", object=" + context.getContextObject());
		}
	}
	...
	public void setParentContext(String contextName, int objectId) {
		...
		if (top == null || !top.getContextName().equals(contextName) || top.getContextObject() != objectId){
	...
	private Stack<ContextView> getStack(){
		Stack<ContextView> stack = (Stack<ContextView>)ActionContext.getContext().getSession().get(contextStackParam);
		if (stack == null){
			stack = new Stack<ContextView>();
			ActionContext.getContext().getSession().put(contextStackParam, stack);
		}
		return stack;
	}
