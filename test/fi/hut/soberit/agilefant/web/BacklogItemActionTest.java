...
public class BacklogItemActionTest extends SpringTestCase {
    ...
    // TODO taskien lisäys, jonka jälkeen effort estimate sums (from tasks) et
    // cetera..
    ...
    /*
     * public void setTaskAction(TaskAction taskAction) { this.taskAction =
     * taskAction; }
     */
     ...
     * this.backlogItemAction.create(); BacklogItem bi =
     * this.backlogItemAction.getBacklogItem();
     * bi.setAssignee(this.getTestUser(1)); bi.setBacklog(b);
     * bi.setDescription("FOOBAR"); bi.setName("FOOBAR");
     * bi.setPriority(TEST_PRI1); bi.setAllocatedEffort(TEST_EST1);
     * bi.setPriority(TEST_PRI1); String result =
     * this.backlogItemAction.store(); assertSame("Backlog item creation
     * failed", Action.SUCCESS, result); Collection<BacklogItem> cb =
     * this.backlogItemDAO.getAll(); for(BacklogItem bb : cb) { return bb; }
     * fail("Backlog item not created as supposed"); return null; }
     */
    ...
    private void fetchForEditing(String name, String desc) {
        ...
        backlogItemAction.setBacklogItemId(this.getBacklogItem(name, desc)
                .getId());
        String result = backlogItemAction.edit();
        super.assertEquals("edit() was unsuccessful", result, Action.SUCCESS);
        super.assertNotNull(
                "Edit() was unsuccesfull (fetched backlog item was null)",
                this.backlogItemAction.getBacklogItem());
    }
    ...
    private BacklogItem getBacklogItem(String name, String desc) {
        for (BacklogItem bi : getAllBacklogItems()) {
            if (bi.getName().equals(name) && bi.getDescription().equals(desc))
                return bi;
        }
        return null;
    }
    ...
    public void testCreate() {
        ...
        super.assertEquals("New user had an invalid id", 0, backlogItemAction
    ...
    public void testStore() {
        ...
        this.setContents(TEST_NAME1, TEST_DESC1, creator, TEST_WATCH1,
    ...
    /**
     * Test storing backlog item when original estimate was null and new
     * estimate has value. Placeholder task must get the new value.
     */
    public void testStoreWithOrigNullEstimate() {
        ...
        this.setContents(TEST_NAME1, TEST_DESC1, creator, TEST_WATCH1,
                assignee, null, TEST_PRI1, TEST_STAT1, backlog);
        String result = backlogItemAction.store();
        storedBI = this.getBacklogItem(TEST_NAME1, TEST_DESC1);
    ...
    public void testStore_withDifferentData() {
        ...
        User assignee = this.getTestUser(1);
        User creator = this.getTestUser(2);
        this.setContents(TEST_NAME2, TEST_DESC2, creator, TEST_WATCH2,
                assignee, TEST_EST2, TEST_PRI2, TEST_STAT2, backlog);
        this.store();
        BacklogItem storedBI = this.getBacklogItem(TEST_NAME2, TEST_DESC2);
        super.assertNotNull("Stored backlog item wasn't found", storedBI);
        this.checkContents("stored backlog item", storedBI, TEST_NAME2,
                TEST_DESC2, creator, TEST_WATCH2, assignee, TEST_EST2,
                TEST_PRI2, TEST_STAT2, backlog);
    }
    ...
    public void testStore_withEmptyName() {
        ...
        String result = backlogItemAction.store();
        super.assertEquals(
                "storing backlog item with name missing was successful",
                Action.ERROR, result);
        assertTrue(
                "backlogitem.missingName -error not found",
                errorFound(backlogItemAction.getText("backlogitem.missingName")));
    }

    /*
     * public void testStoreAndEdit_withNullEstimates() { this.create();
     * BacklogItem bi = this.getTestBacklogItem(this.getTestBacklog()); User
     * assignee = this.getTestUser(1); User creator = this.getTestUser(2);
     * this.setContents(TEST_NAME1, TEST_DESC1, creator, assignee, null,
     * TEST_PRI1, TEST_STAT1, bi); String result = taskAction.store();
     * super.assertEquals("storing task with null estimate was unsuccesfull",
     * Action.SUCCESS, result); this.fetchForEditing(TEST_NAME1, TEST_DESC1);
     * super.assertNull("Null estimate wasn't null after fetching for editing",
     * taskAction.getTask().getEffortEstimate()); this.setEstimate(TEST_EST1);
     * this.store(); this.fetchForEditing(TEST_NAME1, TEST_DESC1);
     * super.assertEquals("Changed estimate wasn't as supposed", TEST_EST1,
     * taskAction.getTask().getEffortEstimate()); this.setEstimate(null);
     * this.store(); this.fetchForEditing(TEST_NAME1, TEST_DESC1);
     * super.assertNull("Setting estimate to null (for a task create before) was
     * unsuccesfull", taskAction.getTask().getEffortEstimate()); }
     */
    ...
    public void testEdit() {
        ...
        this.store();
        backlogItemAction.setBacklogItem(null);
        backlogItemAction.setBacklogItemId(this.getBacklogItem(TEST_NAME1,
                TEST_DESC1).getId());
        String result = backlogItemAction.edit();
        super.assertEquals("edit() was unsuccessful", result, Action.SUCCESS);
        BacklogItem fetchedBI = backlogItemAction.getBacklogItem();
        this.checkContents("backlog item fetched for editing", fetchedBI,
    ...
    public void testEdit_withInvalidId() {
        ...
        String result = backlogItemAction.edit();
        assertEquals("Invalid backlog item id didn't result an error.",
                Action.ERROR, result);
    ...
    public void testStore_withUpdate() {
        ...
        BacklogItem updatedBacklogItem = this.getBacklogItem(TEST_NAME2,
                TEST_DESC2);
        this.checkContents("updated task", updatedBacklogItem, TEST_NAME2,
                TEST_DESC2, creator, TEST_WATCH1, creator, TEST_EST2,
                TEST_PRI2, TEST_STAT2, backlog);
    }
    ...
    public void testDelete() {
        ...
        super.assertEquals(
                "The number of backlog items didn't decrease with delete().",
                n - 1, getAllBacklogItems().size());

        BacklogItem test = this.getBacklogItem(TEST_NAME1, TEST_DESC1);
        super.assertNull("The deleted backlog item wasn't properly deleted",
                test);
    }
