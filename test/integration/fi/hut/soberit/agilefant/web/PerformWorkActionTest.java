 ...
 * <p>
 * To create a performed work event with PerformWorkAction, we need a full fledging
 * hierarchy of a backlog, a backlogitem and a task. The test needs to create 
 * entities for all those. A product is used as the backlog. Creating the test entities 
 * is done in methods beginning with createTest.
 * 
...
public class PerformWorkActionTest extends SpringTestCase {
	 ...
	 */
	private ActivityType createTestActivityType(String name, String description, int percentage) {
		...
		assertTrue("stored activitytype had an invalid id", activityType.getId()>0);
		
		ActivityType result = activityTypeDAO.get(activityType.getId());
		
		assertNotNull("getting stored failed", result);
		assertEquals("stored activitytype had an invalid name", result.getName(), name);
		assertEquals("stored activitytype had an invalid description", result.getDescription(), description);
		assertEquals("stored activitytype had invalid spending percantage", result.getTargetSpendingPercentage(), percentage);
	 ...
	 */	
	private WorkType createTestWorkType(String name, String description, ActivityType activityType) {
		...
		assertEquals("stored worktype had an invalid name", result.getName(), name);
		assertEquals("stored worktype had an invalid description", result.getDescription(), description);
		assertEquals("stored worktype had invalid activitype", result.getActivityType().getId(), activityType.getId());
	...
	private Product createTestProduct(String name, String description) {
		...
		assertEquals("stored product had an invalid name", result.getName(), name);
		assertEquals("stored product had an invalid description", result.getDescription(), description);
	 ...
	 */	
	private BacklogItem createTestBacklogItem(Backlog backlog, String name, String description) {
		...
		assertEquals("stored product had an invalid name", result.getName(), name);
		assertEquals("stored product had an invalid description", result.getDescription(), description);
		assertEquals("stored worktype had invalid backlog", result.getBacklog().getId(), backlog.getId());
	...
	private Task createTestTask(BacklogItem bli, AFTime effortEstimate, String name, String description) {
		...
		assertEquals("stored task had an invalid description", result.getDescription(), description);
		assertEquals("stored task had invalid backlogitem", result.getBacklogItem().getId(), bli.getId());
		assertEquals("stored task had an invalid effort estimate", result.getEffortEstimate(), effortEstimate);
		assertNull("stored task had nonzero performed effort", result.getPerformedEffort());
	...
	private User createTestUser(String login, String name) {
		...
		assertEquals("stored task had an invalid login name", result.getLoginName(), login);
		assertEquals("stored task had an invalid login name", result.getFullName(), name);
	...
	/**
	 * Run the action method execute in performWorkAction, with testing for success. 
	 */
	...
	private boolean checkCollectionContainsId(Collection<? extends TaskEvent> collection, int id) {
	...
	private TaskEvent findEventWithId(Collection<? extends TaskEvent> collection, int id) {
		for(TaskEvent event : collection)
	...
	public void testExecute() {
		...
		// create the entity hierarchy around our work event using createTest - methods
		Product product = createTestProduct(PRODUCT_NAME, PRODUCT_DESCRIPTION);
		BacklogItem bli = createTestBacklogItem(product, BACKLOGITEM_NAME, BACKLOGITEM_DESCRIPTION);
		Task task = createTestTask(bli, new AFTime(AFTime.parse(TASK_EFFORT)), TASK_NAME, TASK_DESCRIPTION);
		ActivityType activityType = createTestActivityType(ACTIVITYTYPE_NAME, ACTIVITYTYPE_DESCRIPTION, ACTIVITYTYPE_PERCENTAGE);
		WorkType workType = createTestWorkType(WORKTYPE_NAME, WORKTYPE_DESCRIPTION, activityType);
		...
		assertEquals("task effort did not increase after new work performed", task.getPerformedEffort(), new AFTime(AFTime.parse(PERFORMWORK_EFFORT)));
		
		int workEventId = performWorkAction.getEvent().getId();
		
		// test that our event appeared in all the collections that can be requested from the DAO
		assertTrue("performed work for the task did not contain added work event", checkCollectionContainsId(performedWorkDAO.getPerformedWork(task), workEventId));
		assertTrue("performed work for the backlogitem did not contain added work event", checkCollectionContainsId(performedWorkDAO.getPerformedWork(bli), workEventId));
		assertTrue("events for the task did not contain added work event", checkCollectionContainsId(task.getEvents(), workEventId));
		
		// since we don't have a getter in the DAO for single events by id, get a fresh copy of our event this way
		PerformedWork ourEvent = (PerformedWork)findEventWithId(task.getEvents(), workEventId);

		// see if the event has proper contents 
		assertEquals("work event had invalid task", ourEvent.getTask().getId(), task.getId());
		assertEquals("work event had invalid effort", ourEvent.getEffort(), new AFTime(AFTime.parse(PERFORMWORK_EFFORT)));
		assertEquals("work event had invalid comment", PERFORMWORK_COMMENT, ourEvent.getComment());
		// eh, test that the difference is no more than 1h :) Where does this value actually get set for an event?
		assertTrue("work event had invalid creation date", Math.abs(current.getTime() - ourEvent.getCreated().getTime()) < 1*60*60*1000 );
		assertEquals("work event had invalid work type", workType.getId(), ourEvent.getWorkType().getId());
		//assertEquals("work event had invalid work type", user.getId(), ourEvent.getActor().getId());
	}
