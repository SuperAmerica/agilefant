...
public class WorkTypeActionTest extends SpringTestCase {
	...
	private void configWorkType(String name, String description, ActivityType activityType) {
	...
	/*
	 * Method for calling workTypeAction.create that is supposed to work (and 
	...
	/*
	 * Method for calling workTypeAction.store that is supposed to work (and 
	...
	private static boolean WorkTypeCollectionContainsId(Collection<WorkType> collection, int id) {
		for(WorkType wt : collection) {
			if(wt.getId() == id) return true;
		}
		return false;
	}
	 ...
	 * @param workTypeAction springed WorkTypeAction object
	...
	public void checkItems(WorkType storedWorkType, ActivityType storedActivityType, ActivityType test) {
		activityTypeDAO.refresh(storedActivityType); 
				
		assertNotNull("worktype was null", storedWorkType);
		assertEquals("worktype had invalid name", storedWorkType.getName(), TEST_NAME);
		assertEquals("worktype had invalid description", storedWorkType.getDescription(), TEST_DESCRIPTION);
		assertEquals("worktype had an invalid activity type", storedWorkType.getActivityType().getId(), test.getId());

		super.assertNotNull("activitytype was null", storedActivityType);
		super.assertEquals("activitytype had invalid id", test.getId(), storedActivityType.getId());
		super.assertTrue("activitytype had invalid name", storedActivityType.getName().equals(ACTIVITYTYPE_TEST_NAME));
		super.assertTrue("activitytype had invalid description", storedActivityType.getName().equals(ACTIVITYTYPE_TEST_DESCRIPTION));
		super.assertTrue("activitytype had invalid worktype-collection", WorkTypeCollectionContainsId(storedActivityType.getWorkTypes(), storedWorkType.getId())); 
	}	
	
	public void checkStoreResult(ActivityType test) {
		WorkType storedWorkType = workTypeDAO.get(workTypeAction.getStoredWorkTypeId());
	...
	public void testCreate(){
		...
		super.assertEquals("New work type had an invalid id", 0, workTypeAction.getWorkTypeId());
		super.assertNotNull("new work type object was null ", workTypeAction.getWorkType());
	...
	public void testStore() {
		...
		super.assertEquals("The total number of stored worktypes didn't grow up with store().", n+1, this.getAllWorkTypes().size());
	...
	public void testStore_withoutCreate() {
		...
		assertTrue("Store without create didn't fail", 
				...
				errorFound(workTypeAction.getText("workType.activityTypeNotFound")) || 
	...
	public void testEdit_withInvalidId() {
		...
		String result = workTypeAction.edit();
		assertEquals("Invalid worktype id didn't result an error.", Action.ERROR, result);
		assertTrue("worktype.notFound -error not found", 
				errorFound(workTypeAction.getText("wotkType.notFound")) || errorFound(workTypeAction.getText("workType.notFound")) ||
				errorFound(workTypeAction.getText("workType.activityTypeNotFound")) || errorFound(workTypeAction.getText("workType.activityTypeNotFound")));
	}
	...
	public void testStore_withUpdate() {
		...
		WorkType updatedWorkType = workTypeDAO.get(workTypeAction.getStoredWorkTypeId());
		
		super.assertNotNull("WorkType wasn't stored properly (wasn't found)", updatedWorkType);		
		super.assertEquals("Updated WorkType had invalid name", updatedWorkType.getName(), TEST_NAME+"2"); 
		super.assertEquals("Updated WorkType had invalid name", updatedWorkType.getDescription(), TEST_DESCRIPTION+"2");
		super.assertEquals("Updated WorkType had invalid activitytype", updatedWorkType.getActivityType().getId(), test.getId());
	...
	public void testDelete() {
		...
		super.assertEquals("The number of users didn't decrease with delete().", n-1, getAllWorkTypes().size());
