...
public class ContextViewInterceptor implements Interceptor {
	...
	public String intercept(ActionInvocation invocation) throws Exception {
		if ("true".equals(this.getRequestParameter(invocation, RESET_CONTEXT_PARAM))){
			contextViewManager.resetContext();
		}
		
		int objectId = 0;
		String viewName = this.getRequestParameter(invocation, CONTEXT_VIEW_NAME_PARAM);
		try{
			objectId = Integer.parseInt(this.getRequestParameter(invocation, CONTEXT_OBJECT_ID_PARAM));
	...
	private String getRequestParameter(ActionInvocation invocation , String name){
		Object value = invocation.getInvocationContext().getParameters().get(name);
		if (value == null){
			return null;
