...
<c:forEach items="${projects}" var="pro">
	...
	<div id="subItemHeader">
		...
		<ww:a href="%{parentActionUrl}&contextViewName=dailyWork">
			<u><c:out value="${pro.product.name}:" /></u>
		</ww:a>
...
<display:table class="dailyWorkProject" name="${bliMap[pro]}"
	id="item2" requestURI="${currentAction}.action" >
	...
	<display:column title="State" sortable="false" class="taskColumn">
			...
			<c:choose>
				<c:when test="${!(empty item2.tasks)}">
					...
					<c:out value="${fn:length(item2.tasks)}" /> tasks, <aef:percentDone
				...
				<c:otherwise>
					<a href="javascript:toggleDiv(${divId});" title="Click to expand">
				    <ww:text name="task.state.${item2.state}" /><br />
